...
## save the non-resolved version
wd_scenarios_emissions_org <- wd_scenarios_emissions
...
wd_scenarios_emissions <- wd_scenarios_emissions %>%
  ...
  filter(subsector!="Total") %>%
  group_by(id,model,scenario,category,gas,sector,subsector,year) %>%
  summarise(value_gwp=sum(value_gwp,na.rm=TRUE))

# sector inconsistencies
wd_scenarios_emissions <- full_join(wd_scenarios_emissions,wd_sector_totals,by = join_by(id, model, scenario, year, gas,sector))

wd_scenarios_emissions <- wd_scenarios_emissions %>%
  mutate(value_gwp=ifelse(is.na(subsector),sector_total,value_gwp)) %>%
  mutate(subsector=ifelse(is.na(subsector),"Other",subsector))

residuals <- wd_scenarios_emissions %>%
  group_by(id,model,scenario,year,gas,sector) %>%
...
wd_scenarios_emissions <- rbind(wd_scenarios_emissions,residuals)


## gas inconsistencies
wd_scenarios_emissions <- left_join(wd_scenarios_emissions %>% select(-sector_total),wd_gas_totals,by = join_by(id, model, scenario, year, gas))
...
wd_scenarios_emissions <- wd_scenarios_emissions %>% 
  group_by(id,model,scenario,category,gas,sector,subsector,var,year) %>% 
  summarise(value_gwp=sum(value_gwp,na.rm=TRUE))
...
test_residual <- wd_scenarios_emissions %>%
  #filter(year==2050) %>%
  filter(gas!="Kyoto Gases") %>%
  group_by(id,year) %>%
  summarise(recalculated_net_emissions=sum(value_gwp,na.rm=TRUE))

test_residual <- left_join(test_residual,wd_scenarios_emissions %>% ungroup %>% filter(gas=="Kyoto Gases") %>% select(id,year,native_net_emissions=value_gwp),by = join_by(id,year))

test_residual <- left_join(test_residual,
                           wd_scenarios_emissions %>% 
                             ungroup %>%
                             filter(gas=="CO2") %>%
                            group_by(id,year) %>%  summarise(recalulated_net_CO2_emissions=sum(value_gwp,na.rm=TRUE)) %>% 
                             select(id,year,recalulated_net_CO2_emissions),
                          by = join_by(id,year))
...
test_residual <- left_join(test_residual,cc_scenarios %>% select(id) %>% mutate(include=1)) %>% 
  filter(include==1) %>% 
  select(-include)

test_residual_stat <- test_residual %>%
  group_by(year) %>%
...
wd_scenarios_emissions <- spread(wd_scenarios_emissions %>% 
                  ungroup() %>% 
...
wd_scenarios_emissions <- gather(wd_scenarios_emissions,var,value,-id,-model,-scenario,-category,-year)
wd_scenarios_emissions <- left_join(wd_scenarios_emissions,cc_sectors,by="var")
wd_scenarios_emissions <- wd_scenarios_emissions %>% 
  select(id,model,scenario,category,gas,sector,subsector,var,year,value)
...
wd_scenarios_emissions <- left_join(wd_scenarios_emissions,cc_netzero,by="id")
...
no_scenarios <- wd_scenarios_emissions %>% 
  select(id) %>%
  distinct()
...
# test_residual <- left_join(test_residual,cc_netzero,by="id")
# test_residual <- left_join(test_residual,wd_scenarios_emissions_org %>% 
#                              filter(var=="AR6 climate diagnostics|Infilled|Emissions|CO2") %>% 
#                              select(id,year,native_CO2_emissions=value_gwp),
#                            by = join_by(id, year))
# 
# test_nz_year <- test_residual %>%  
#   select(id,year,native_CO2_emissions,recalulated_net_CO2_emissions,native_net_zero_year=net_zero_CO2) %>% 
#   filter(recalulated_net_CO2_emissions<0) %>% 
#   group_by(id) %>% 
#   mutate(recalculated_net_zero_year=first(year))
# 
# blarg <- test_nz_year %>% 
#   mutate(difference=recalculated_net_zero_year-native_net_zero_year) %>% 
#   select(id,difference) %>% 
#   distinct() %>% 
#   filter(abs(difference)>5)
# 
# test_nz_year <- left_join(test_residual %>% 
#                             select(id,year,native_CO2_emissions,recalulated_net_CO2_emissions,native_net_zero_year=net_zero_CO2),
#                           blarg,by="id") %>% 
...
wd_fuels <- left_join(wd_fuels,cc_netzero,by="id") %>% 
  filter(!is.na(net_zero_CO2)) %>% 
  mutate(include=ifelse(net_zero_CO2==year,1,0)) %>% 
...
wd_fuels_check <- median_and_range(wd_fuels_check,c("year"))
...
writeData(wb_figures, sheet = "fig_2c_fuels",spread(wd_fuels %>% select(-fuel),var,value) %>% 
            select(-net_zero_CO2,-net_zero_GHG) %>% 
...
```{r plot_SoCDR_Ch7_b,fig.height=4,fig.width=7,fig.path="Results/",dev=c('png', 'pdf', 'svg'),dpi=300}
...
saveWorkbook(wb_figures,"Results/figure_data.xlsx",overwrite=T)
saveWorkbook(wb_analysis,"Results/analysis.xlsx",overwrite=T)
#saveWorkbook(wb_data,"Results/processed_data.xlsx",overwrite=T)


# write.csv(wd_scenarios_emissions %>%
#              filter(var!="Kyoto Gases") %>%
#              filter(!grepl("Carbon Sequestration",var)),
#           file="residual-emissions-ERL-processed-data.csv",sep=";")
