...
# 
# test_residual <- left_join(test_residual,wd_scenarios_emissions %>% ungroup %>% filter(year==2050) %>% filter(gas=="Kyoto Gases") %>% select(id,kyoto_gas_total=value))
# 
# test_residual <- test_residual %>% 
#   mutate(difference=value-kyoto_gas_total) %>% 
...
```{r plot_principal_components,fig.height=4,fig.width=6}
...
```{r cluster_analysis,fig.height=6,fig.width=8,fig.path="../Results/",dev=c('png', 'pdf', 'svg'),dpi=300}
...
wd_subsectors_total <- wd_subsectors %>% 
  mutate(model="All") %>% 
  mutate(year=ifelse(year!=2020,2050,year)) %>%
